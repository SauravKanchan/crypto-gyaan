<script>
    ...
    onMount(async () => {
        address = await window.wallet.getAddress();
        balance = ethers.utils.formatEther(String(await window.wallet.getBalance()));
        total_tokens = parseInt(await window.erc721.functions.balanceOf(window.wallet.address));
        token_exist = Boolean(total_tokens);
        for (let i = 0; i < total_tokens; i++) {
            add_token(parseInt(await window.erc721.functions.tokenOfOwnerByIndex(window.wallet.address, i)));
        }
...
<div class="row">
    ...
    {:else}
        {#if total_tokens}
            <div class="h2 strong">There is no gyaan to buy or it is yet to be fetched from blockchain.</div>
        {:else}
            <div class="h2 strong">There is no gyaan to buy.</div>
        {/if}
    {/if}
</div>
