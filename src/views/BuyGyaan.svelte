<script>
    ...
    onMount(async () => {
        ...
        balance = parseFloat(ethers.utils.formatEther(String(await wallet.getBalance())));
                ...
                {#each my_tokens as token, i}
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            {#if balance >= token_price[i] }
                                <button type="button" data={orders[i]} class="btn btn-success" on:click={buy_order}>
                                    Buy
                                </button>
                            {:else}
                                <p class="card-text">Insufficient funds</p>
                            {/if}
                        </div>
                    </div>
                {/each}
